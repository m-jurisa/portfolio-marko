  ...
  return (
    <section className="c-space my-20" id="projects">
      ...
      <div className="grid lg:grid-cols-2 grid-cols-1 mt-12 gap-5 w-full">
        ...
        {/* Replace 3D viewer with a simple video preview when available */}
        <div className="border border-black-300 bg-black-200 rounded-lg h-96 md:h-full flex items-center justify-center p-4">
          {currentProject.texture?.endsWith('.mp4') ? (
            <video src={currentProject.texture} className="w-full h-full object-contain rounded-lg" autoPlay loop muted playsInline />
          ) : (
            <img src={currentProject.texture} className="w-full h-full object-contain rounded-lg" alt="project" />
          )}
        </div>
      </div>
    </section>
