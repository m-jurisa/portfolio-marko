export default function Testimonials({ items = [], title, subtitle, locale = 'de' }) {
  if (!items.length) return null

  const t = {
    de: {
      title: 'Referenzen',
      subtitle: 'Was Projektpartner und Kunden über die Zusammenarbeit sagen.',
    },
    en: {
      title: 'Testimonials',
      subtitle: 'What clients and partners say about working together.',
    },
  }[locale] || { title: 'Testimonials', subtitle: '' }
  ...
  return (
    <section className="c-space my-16">
      <div className="mb-8">
        <h3 className="grid-headtext mb-4">{title ?? t.title}</h3>
        <p className="grid-subtext max-w-2xl">{subtitle ?? t.subtitle}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {items.map((testimonial, idx) => (
          <blockquote key={idx} className="group grid-container hover:border-purple-500/30 transition-all duration-300">
